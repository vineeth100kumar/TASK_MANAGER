...
def serialize_item(row: aiosqlite.Row, subtasks: Optional[List[Dict[str, Any]]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "repeat_until": _column(row, "repeat_until"),
        "repeat_count": _column(row, "repeat_count"),
        "repeat_done": _column(row, "repeat_done", 0) or 0,
        "next_occurrence": _column(row, "next_occurrence"),
        "project_id": _column(row, "project_id"),
        "milestone_id": _column(row, "milestone_id"),
        "location": _column(row, "location"),
        "is_all_day": bool(_column(row, "is_all_day", 0)),
        "position": _column(row, "position"),
        "estimated_minutes": _column(row, "estimated_minutes"),
