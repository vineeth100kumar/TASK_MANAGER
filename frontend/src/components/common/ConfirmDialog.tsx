...
export const ConfirmDialog: React.FC<ConfirmDialogProps> = ({
  ...
  return (
    ...
    >
      <div 
        className="w-full max-w-md clipping clipping-white border-2 border-stone-300 dark:border-stone-700 rounded-none p-5 sm:p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-150 relative"
      ...
      >
        ...
        <div className="flex items-start gap-3.5 pt-2">
          ...
          <div className="flex-1 min-w-0">
            <h3 id="confirm-dialog-title" className="font-editorial text-base sm:text-lg font-bold text-stone-900 dark:text-stone-100 tracking-tight">
              {title}
            </h3>
            <div id="confirm-dialog-message" className="font-editorial text-xs text-stone-600 dark:text-stone-300 mt-1 leading-relaxed">
              {message}
            </div>
          </div>
          <button
            onClick={onCancel}
            className="text-stone-500 hover:text-stone-800 dark:hover:text-stone-300 p-1 border border-stone-300 dark:border-stone-800 hover:bg-black/5 dark:hover:bg-stone-800 rounded-none transition-colors"
        ...
        <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-stone-200 dark:border-stone-800">
          <button
            ...
            onClick={onCancel}
            disabled={isLoading}
            className="px-4 py-2 rounded-none text-xs font-ledger uppercase tracking-wider text-stone-700 dark:text-stone-300 hover:text-stone-900 dark:hover:text-white bg-paper-aged dark:bg-stone-900 hover:bg-stone-200 dark:hover:bg-stone-800 border border-stone-300 dark:border-stone-700 transition-colors disabled:opacity-50"
          >
            {cancelText}
          </button>
          <button
