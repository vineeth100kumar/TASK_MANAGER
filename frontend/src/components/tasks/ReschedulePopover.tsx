...
export const ReschedulePopover: React.FC<ReschedulePopoverProps> = ({
  ...
  return (
    <div
      ref={popoverRef}
      className="absolute z-50 mt-1 w-56 rounded-none bg-paper-white dark:bg-[#181612] border border-ink-base/25 dark:border-paper-light/20 shadow-2xl p-2 space-y-1 text-xs animate-in fade-in zoom-in-95 duration-100"
      onClick={e => e.stopPropagation()}
    >
      <div className="px-2 py-1 text-[9px] font-ledger font-bold text-ink-muted dark:text-stone-400 uppercase tracking-widest flex items-center justify-between border-b border-ink-base/10 dark:border-stone-800 pb-1.5 mb-1">
        <span>RESCHEDULE CLIPPING</span>
      ...
      <div className="space-y-0.5">
        {options.map((opt) => (
          <button
            ...
            className={`w-full flex items-center justify-between px-2.5 py-1.5 rounded transition-colors text-left font-editorial ${
          ...
          >
            ...
            <span className="text-[10px] text-ink-muted dark:text-stone-400 font-ledger">{opt.sub}</span>
          </button>
        ))}
      </div>
      ...
      <div className="pt-1.5 border-t border-ink-base/10 dark:border-stone-800 space-y-1">
        {/* Custom date input */}
        <label className="flex items-center space-x-2 px-2.5 py-1.5 rounded hover:bg-paper-aged/70 dark:hover:bg-stone-800/70 transition-colors cursor-pointer text-ink-base dark:text-stone-300 font-editorial">
          <CalendarCheck className="w-3.5 h-3.5 text-ink-muted dark:text-stone-400" />
          <span className="flex-1 text-xs">Calendar Stamp...</span>
          <input
            type="date"
        ...
        {currentDueDate && (
          <button
            ...
            className="w-full flex items-center space-x-2 px-2.5 py-1.5 rounded text-rose-700 dark:text-rose-400 hover:bg-rose-500/10 transition-colors text-left font-ledger text-[11px] uppercase font-bold"
          >
